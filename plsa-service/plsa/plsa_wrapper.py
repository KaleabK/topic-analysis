...
class PLSA_wrapper:
    ...
    def generate_topics_json(self):
        ...
        os.mkdir(self.plsa_parameters_path + self.unique_folder_naming)
...
def run_plsa():
    ...
    #path = str(pathlib.Path(os.path.abspath('')).parents[1])+'/appData/misc/extracted.json'
    ...
    s = PLSA_wrapper(docs[0],local=True)
    #s.unique_folder_naming = unique_folder_naming
    s.num_topics = docs[1]
    s.topic_divider = docs[2]
    s.max_iter = docs[3]
    s.beta = docs[4]
    s.write_to_json()
