...
class pLSA(object):
    ...
    def __init__(self, model=None):
        ...
        if os.path.exists("random.hdf5"):
            os.remove("random.hdf5")
        self.f =  h5py.File('random.hdf5', 'a')
    ...
    def train(self, td, Z, maxiter=maxiter2, eps=eps2, folding_in=False,beta=beta2,min_iteration = min_iteration2): # eps=0.01 is default value
        ...
        p_d_z_old = np.zeros_like(self.f['p_d_z'])
        p_w_z_old = np.zeros_like(self.f['p_w_z'])
        p_z_old = np.zeros_like(self.f['p_z'])
        self.f.create_dataset("p_d_z_old", data=p_d_z_old)
        self.f.create_dataset("p_w_z_old", data=p_w_z_old)
        self.f.create_dataset("p_z_old", data=p_z_old)
        ...
        train_func(td.astype(np.uint32),self.f['p_z'][:],self.f['p_w_z'][:],self.f['p_d_z'][:],
                   self.f['p_z_old'][:],self.f['p_w_z_old'][:],self.f['p_d_z_old'][:],
                   maxiter, eps,beta,min_iteration,
                   folding_in, self.debug,self.logL_c )
    ...
    def topic_document(self):
        ...
        self.p_z_d = self.f['p_z'][:] * self.f['p_d_z'][:]
        self.f.create_dataset("p_z_d",data=self.p_z_d)
        # print('p_z_d-----')
        # print(self.p_z_d.shape)
        # print('p_z_d.T-----')
        # print(self.p_z_d.T.shape)

        return self.f['p_z_d'][:].T # T is for transpose
    ...
    def document_topics(self):
        ...
        return normalize((self.f['p_d_z'][:] * self.f['p_z'][np.newaxis,:]).T, axis=0)
    ...
    def word_topics(self):
        ...
        return normalize((self.f['p_w_z'][:] * self.f['p_z'][np.newaxis,:]).T, axis=0)
    ...
    def unigram_smoothing(self):
        ...
        """
        V, Z = self.f['p_w_z'][:].shape
        D, Z = self.f['p_d_z'][:].shape
        p_w_d = np.zeros((V,D), dtype=np.double)
        self.f.create_dataset("p_w_d", data=p_w_d)
        for d in range(D):
            for w in range(V):
                self.f['p_w_d'][w,d] = np.sum(self.f['p_w_z'][w,:] * self.f['p_d_z'][d,:])
        return p_w_d
    ...
    def folding_in(self, d, maxiter=50, eps=0.01):
        ...
        plsa.p_z = self.f['p_z'][:]
        plsa.p_w_z = self.f['p_w_z'][:]
        plsa.p_z_d= self.f['p_z_d'][:]
        plsa.train(d[:,np.newaxis], Z, maxiter, eps, folding_in=True)
        return normalize(self.f['p_z'][:] * plsa.p_d_z[:,0])
    ...
    def global_weights(self, gw):
        ...
        """
        return np.sum(gw[:,np.newaxis] * self.f['p_w_z'][:], axis=0)
